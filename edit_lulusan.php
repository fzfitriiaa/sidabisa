...
$id = $_GET["id"];
$q2 = mysqli_query($db, " SELECT * FROM tb_keterserapan WHERE id=".$id." ");
while ($hasil = mysqli_fetch_array($q2)){
    $id=$hasil["id"];
    $npsn=$hasil["npsn"];
    $bkk=$hasil["bkk"];
    $jml_lulusan_txt=$hasil["jml_lulusan_txt"];
    $jml_lulusan=$hasil["total_lulusan"];
    $jml_industri=$hasil["jml_industri"];
    $jml_univ=$hasil["jml_univ"];
    $jml_wirausaha=$hasil["jml_wirausaha"];
    $jml_nganggur=$hasil["jml_nganggur"];
    $terserapan=$hasil["jml_serapan"];
    $jml_sesuai=$hasil["jml_sesuai_kompetensi"];
    $jml_tidak=$hasil["jml_tdk_sesuai_kompetensi"];
    $kerjasama=$hasil["kerjasama_industri"];
    }
?>
...
if (isset($_POST["submit"])) {
    ...
    if ($valid){
        ...
        $jml_lulusan_txt = $db->real_escape_string($jml_lulusan_txt);
        $terserapan = $db ->real_escape_string($terserapan);
        $kerjasama = $db->real_escape_string($kerjasama);
        
        $query = "UPDATE tb_keterserapan SET 
                    npsn=$npsn, 
                    bkk='".$bkk."',
                    jml_lulusan_txt='".$jml_lulusan_txt."',
                    total_lulusan=$jml_lulusan,
                    jml_industri=$jml_industri,
                    jml_univ=$jml_univ,
                    jml_wirausaha=$jml_wirausaha,
                    jml_nganggur=$jml_nganggur,
                    jml_serapan='".$terserapan."',
                    jml_sesuai_kompetensi=$jml_sesuai,
                    jml_tdk_sesuai_kompetensi=$jml_tidak,
                    kerjasama_industri='".$kerjasama."'
                  WHERE id='".$id."'";
        #execute query
        ...
        if (!$result) {
            ...
            echo "<script>window.alert('Data berhasil diedit'); window.location='lulusan.php'</script>";
        }
        #close connection
        $db->close();
    }
                        ...
                        <div class="d-flex align-items-center">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb m-0 p-0">
                                    <li class="breadcrumb-item"><h2>Form Edit Keterserapan Lulusan</h2></li>
                                </ol>
                            </nav>
                        </div>
                    </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="bkk"
                                        id="bkk" value="sudah" <?php if($bkk=='sudah') echo 'checked';?>>
                                    <label class="form-check-label" for="bkk">Sudah</label>
                            </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="bkk"
                                        id="bkk" value="belum" <?php if($bkk=='belum') echo 'checked';?>>
                                    <label class="form-check-label" for="bkk">Belum</label>
                                </div>
                                <div class="error"><?php if (isset($error_bkk)) echo $error_bkk;?></div>
                            </div>
                    </div>
                    <div class="form-group">
                        <label for="jml_lulusan_txt" class="col-lg-9" style="vertical-align: middle;">Berapa tamatan lulusan tiap program keahlian saudara?</label>
                            <div class="col-md-6">
                                <input type="text" class="form-control" id="jml_lulusan_txt" name="jml_lulusan_txt" value="<?php echo $jml_lulusan_txt; ?>">
                                <div class="error"><?php if (isset($error_jml_lulusan_txt)) echo $error_jml_lulusan_txt;?></div>
                            </div>
                </div>
                    <div class="form-group">
                        <label for="jml_industri" class="col-lg-9" style="vertical-align: middle;">Jumlah Tamatan Lulusan yang masuk ke industri, </label>
                            <div class="col-md-6">
                                <input type="text" class="form-control" id="jml_industri" name="jml_industri" value="<?php echo $jml_industri; ?>">
                                <div class="error"><?php if (isset($error_jml_industri)) echo $error_jml_industri;?></div>
                            </div>
                    </div>
                    <div class="form-group"> 
                        <label for="jml_univ" class="col-lg-9" style="vertical-align: middle;">Tamatan lulusan yang masuk universitas, </label>
                            <div class="col-md-6">
                                <input type="text" class="form-control" id="jml_univ" name="jml_univ" value="<?php echo $jml_univ; ?>">
                                <div class="error"><?php if (isset($error_jml_univ)) echo $error_jml_univ;?></div>
                            </div>
                    </div>
                    <div class="form-group"> 
                            <label for="jml_wirausaha" class="col-lg-9" style="vertical-align: middle;">Tamatan lulusan yang berwirausaha, </label>
                                    <div class="col-md-6">
                                        <input type="textarea" class="form-control" id="jml_wirausaha" name="jml_wirausaha" value="<?php echo $jml_wirausaha; ?>">
                                        <div class="error"><?php if (isset($error_jml_wirausaha)) echo $error_jml_wirausaha;?></div>
                                    </div>
                    </div>
                    <div class="form-group"> 
                            <label for="jml_nganggur" class="col-lg-9" style="vertical-align: middle;">Tamatan lulusan yang menganggur </label>
                                    <div class="col-md-6">
                                        <input type="textarea" class="form-control" id="jml_nganggur" name="jml_nganggur" value="<?php echo $jml_nganggur; ?>">
                                        <div class="error"><?php if (isset($error_jml_nganggur)) echo $error_jml_nganggur;?></div>
                                    </div>
                    </div>
                    <div class="form-group">
                        <label for="jml_industri" class="col-lg-9" style="vertical-align: middle;">Total jumlah lulusan </label>
                            <div class="col-md-6">
                                <input type="text" class="form-control" id="total_lulusan" name="total_lulusan" value="<?php echo $jml_lulusan; ?>">
                                <div class="error"><?php if (isset($error_jml_lulusan)) echo $error_jml_lulusan;?></div>
                            </div>
                    </div>
                    <div class="form-group"> 
...
Universitas=20
Wirausaha=5
Menganggur=5"><?php echo $terserapan; ?></textarea>
                                <div class="error"><?php if (isset($error_terserapan)) echo $error_terserapan;?></div>
                            </div>
                    </div>
                    <div class="form-group"> 
                        <label for="jml_lulusan" class="col-lg-9" style="vertical-align: middle;">Berapa jumlah lulusan yang diterima di industri yang sesuai kompetensi? </label>
                            <div class="col-md-6">
                                <input type="textarea" class="form-control" id="jml_sesuai_kompetensi" name="jml_sesuai_kompetensi" value="<?php echo $jml_sesuai; ?>">
                                <div class="error"><?php if (isset($error_jml_sesuai)) echo $error_jml_sesuai;?></div>
                            </div>
                    </div>
                    <div class="form-group"> 
                        <label for="jml_lulusan" class="col-lg-9" style="vertical-align: middle;">Berapa jumlah lulusan yang diterima di industri yang tidak sesuai kompetensi? </label>
                            <div class="col-md-6">
                                <input type="textarea" class="form-control" id="jml_tdk_sesuai_kompetensi" name="jml_tdk_sesuai_kompetensi" value="<?php echo $jml_tidak; ?>">
                                <div class="error"><?php if (isset($error_jml_tidak)) echo $error_jml_tidak;?></div>
                            </div>
                    </div>
                    <div class="form-group"> 
                        <label for="jml_lulusan" class="col-lg-9" style="vertical-align: middle;">Industri yang diajak kerjasama</label>
                            <div class="col-md-6">
                                <input type="textarea" class="form-control" id="kerjasama_industri" name="kerjasama_industri" value="<?php echo $kerjasama; ?>">
                                <div class="error"><?php if (isset($error_kerjasama)) echo $error_kerjasama;?></div>
                            </div>
                    </div>
